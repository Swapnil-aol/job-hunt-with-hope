...
- **Export & share controls** — the template's **Save as PDF** button opens a document+style chooser (`#export-modal`): document (portfolio or résumé), a style, and — for the portfolio — a **Layout** choice: **Continuous (recommended)** — the PDF looks exactly like the page, one tall page — or **Paginated** for paper. The PDF opens with a clickable **Contents** strip that jumps to each section. **Share** opens a Copy link / LinkedIn / X / WhatsApp / Email menu (`#share-menu`). These ship in the template; your job is the content they depend on (resume view, OG meta, share cards).
...
- **Verify the PDF export — produce and inspect, don't assume.** When `python3` is available, run the bundled checker against the generated file: `python3 "$PLUGIN_ROOT/scripts/verify_portfolio_pdf.py" <portfolio.html>`. Read its PASS/FAIL table and fix any FAIL before handing the file over. If `python3` isn't available, say so plainly instead of claiming the export was verified.
...
5. **Point out Share & Save as PDF.** The portfolio carries a **Share** button that opens a small share menu — **Copy link · LinkedIn · X · WhatsApp · Email** (the social links go live once published; Copy keeps its "Copied!" feedback) — and a **Save as PDF** button that now opens an **export chooser** first: pick the document (**portfolio or résumé**), a style (portfolio: classic / ink / showcase · résumé: classic / modern / compact), and — for the portfolio — a **Layout**: **Continuous (recommended)** — the PDF looks exactly like the page, one tall page — or **Paginated** for paper. The PDF opens with a clickable **Contents** strip that jumps to each section. Tell the user plainly: "Save as PDF asks portfolio-or-résumé, a style, and a layout — Continuous makes the PDF look exactly like the page, and the résumé styles print an ATS-clean document from this same file, no separate résumé needed." Your last choice is remembered for next time. Name both buttons so the user knows they're there.
